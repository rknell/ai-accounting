...
void main() {
  group('🛡️ DUPLICATE DETECTION FORTRESS', () {
    ...
    group('🎯 CORE DUPLICATE DETECTION', () {
      test('🛡️ REGRESSION: Categorized transaction is detected as duplicate of uncategorized', () {
        // The core issue: after categorization, the transaction should still be recognized as a duplicate
        expect(uncategorizedTransaction.isSameBankTransaction(categorizedTransaction), isTrue,
            reason: 'Categorized transaction must be recognized as duplicate of original import');
      });

      test('🛡️ REGRESSION: Uncategorized transaction is detected as duplicate of categorized', () {
        // Reverse direction should also work
        expect(categorizedTransaction.isSameBankTransaction(uncategorizedTransaction), isTrue,
            reason: 'Original import must be recognized as duplicate of categorized transaction');
      });

      test('🛡️ REGRESSION: Identical bank transactions with different account codes are duplicates', () {
        // Multiple categorizations of the same bank transaction should be detected
        expect(categorizedTransaction.isSameBankTransaction(identicalTransaction), isTrue,
            reason: 'Same bank transaction with different account codes must be detected as duplicate');
      });

      test('🚫 EDGE_CASE: Standard equality operator still distinguishes account codes', () {
        // The standard == operator should still work for exact matches
        expect(uncategorizedTransaction == categorizedTransaction, isFalse,
            reason: 'Standard equality must distinguish different account codes');
    ...
    group('⚔️ MATCHING CRITERIA VALIDATION', () {
      test('🎯 FEATURE: Different bank accounts are NOT duplicates', () {
        final differentBankTransaction = GeneralJournal(
          ...
          description: 'Visa Purchase 17Dec 7-Eleven 4210 Ormeau Ormeau',
          debits: [SplitTransaction(accountCode: '506', amount: 130.48)],
          credits: [SplitTransaction(accountCode: '002', amount: 130.48)], // Different bank account
          bankBalance: 232939.4,
          notes: 'Different bank account',
        );

        expect(categorizedTransaction.isSameBankTransaction(differentBankTransaction), isFalse,
            reason: 'Different bank accounts must not be considered duplicates');
      });
      ...
      test('🎯 FEATURE: Different dates are NOT duplicates', () {
        ...
        expect(categorizedTransaction.isSameBankTransaction(differentDateTransaction), isFalse,
      ...
      test('🎯 FEATURE: Different amounts are NOT duplicates', () {
        final differentAmountTransaction = GeneralJournal(
          date: testDate,
          description: 'Visa Purchase 17Dec 7-Eleven 4210 Ormeau Ormeau',
          debits: [SplitTransaction(accountCode: '506', amount: 131.48)], // Different amount
          credits: [SplitTransaction(accountCode: '001', amount: 131.48)],
          bankBalance: 232939.4,
          notes: 'Different amount',
        );

        expect(categorizedTransaction.isSameBankTransaction(differentAmountTransaction), isFalse,
      ...
      test('🎯 FEATURE: Different descriptions are NOT duplicates', () {
        final differentDescriptionTransaction = GeneralJournal(
          date: testDate,
          description: 'Visa Purchase 17Dec Woolworths Ormeau', // Different description
          debits: [SplitTransaction(accountCode: '506', amount: 130.48)],
          credits: [SplitTransaction(accountCode: '001', amount: 130.48)],
          bankBalance: 232939.4,
        ...
        expect(categorizedTransaction.isSameBankTransaction(differentDescriptionTransaction), isFalse,
            reason: 'Different descriptions must not be considered duplicates');
      });
    });

    group('🔢 IDENTICAL TRANSACTIONS COUNT VALIDATION', () {
      test('🎯 FEATURE: Multiple identical transactions on same date can coexist', () {
        ...
        expect(firstTransaction.isSameBankTransaction(secondTransaction), isTrue,
            reason: 'Identical transactions on same date should be recognized as same bank transaction type');
        ...
        expect(firstTransaction == secondTransaction, isTrue,
            reason: 'Identical transactions should be exactly equal regardless of bank balance');
      });

      test('🛡️ REGRESSION: Bank balance differences do not affect duplicate detection', () {
        ...
        expect(categorizedTransaction.isSameBankTransaction(sameTransactionDifferentBalance), isTrue,
            reason: 'Bank balance differences must not affect duplicate detection');
      });
    });

    group('🏦 BANK ACCOUNT CODE VALIDATION', () {
      test('🎯 FEATURE: Bank account codes in 0-99 range are properly identified', () {
      ...
      test('🚫 EDGE_CASE: Non-bank account codes are not considered bank codes', () {
        ...
        final nonBankTransaction = GeneralJournal(
          ...
          description: 'Test transaction',
          debits: [SplitTransaction(accountCode: '506', amount: 100.00)],
          credits: [SplitTransaction(accountCode: '999', amount: 100.00)], // Not a bank account
          bankBalance: 1000.00,
          notes: 'Test',
        );

        expect(() => nonBankTransaction.bankCode, throwsException,
            reason: 'Non-bank account codes should throw exception when accessing bankCode');
      });
    });
  });
}
