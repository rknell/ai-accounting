...
const _fastServerThresholdMs =
    1500; // Allow minor scheduler noise while ensuring it's fast.
  ...
  group('⚡ MCP Tools List Timeout Tests', () {
    ...
    test('🚀 FEATURE: Tools list request times out after 3 seconds', () async {
      ...
      try {
        ...
        print(
            '✅ Tools list correctly timed out after ${stopwatch.elapsedMilliseconds}ms');
      }
    }, timeout: Timeout(Duration(seconds: 10)));

    test('🛡️ REGRESSION: Normal tool execution still uses default timeout',
        () async {
      ...
      if (tools.isNotEmpty) {
        ...
        try {
          ...
          final result = await mcpClient.executeTool(
              testTool.function.name, jsonEncode({}));

          stopwatch.stop();

          // Should complete successfully (not timeout after 3 seconds)
          expect(stopwatch.elapsedMilliseconds,
              lessThan(10000)); // Should be much faster than 30s
    ...
    test('🎯 EDGE_CASE: Very fast tools list response is handled correctly',
        () async {
      ...
      try {
        ...
        print(
            '✅ Fast tools list completed in ${stopwatch.elapsedMilliseconds}ms');
      } catch (e) {
        stopwatch.stop();
