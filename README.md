...
- **Supplier spend breakdown** – edit `config/supplier_spend_types.json` to tag suppliers as fixed, variable, or one-time; quarterly report runs generate `supplier_spend_*.html` so you can track cadence shifts.
...
### Supplier spend type mapping
- Configure `config/supplier_spend_types.json` to map supplier names to `fixed`, `variable`, or `one_time`.
- The Supplier Spend Type report (generated by `dart run bin/generate_reports.dart`) analyses every transaction and shows totals + supplier breakdowns for each cadence bucket.
- Update the mapping whenever you onboard a new vendor so the report can classify its spend without manual edits.
...
- Option 7 exports a single-line-per-transaction summary (bank + GST lines removed) so you can quickly scan that every transaction has a meaningful category.
- The MCP accountant server exposes `run_ai_accounting_menu_action`, giving the AI accountant agent direct tool access to run any of these menu workflows (imports, categorisation, exports, or mapping updates) on demand.
